...
p_valDecl = \case
  ...
  PatBind _ pat multAnn grhss ->
    p_match
      ...
      multAnn
      NoSrcStrict
      [L (getLoc pat) $ VisPat NoExtField pat] -- TODO ?
      grhss
  ...
  indentBody <- case NE.nonEmpty m_pats of
    ...
    Just ne_pats@(head_pat :| tail_pats) -> do
      ...
      switchLayout [combinedSpans] $ do
        let stdCase = sep breakpoint (located' p_argPat) m_pats
        case style of
          Function name ->
            p_infixDefHelper
              isInfix
              indentBody
              (p_rdrName name)
              (located' p_argPat <$> m_pats)
          PatternBind -> stdCase
          Case -> stdCase
          Lambda -> do
            let needsSpace = case unLoc head_pat of
                  VisPat _ pat -> case unLoc pat of
                    LazyPat _ _ -> True
                    BangPat _ _ -> True
                    SplicePat _ _ -> True
                    _ -> False
          ...
          LambdaCase -> do
            ...
            unless (null tail_pats) $ do
              ...
              inci $ sep breakpoint (located' p_argPat) tail_pats
      return indentBody
...
p_hsExpr' isApp s = \case
  ...
  HsEmbTy _ HsWC {hswc_body} -> located hswc_body p_hsType -- TODO (type ...)
...
p_argPat :: ArgPat GhcPs -> R ()
p_argPat = \case
  VisPat NoExtField pat -> located pat p_pat
...
p_pat = \case
  ...
  EmbTyPat _ tyPat -> p_tyPat tyPat
