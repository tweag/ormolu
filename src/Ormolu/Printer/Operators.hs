...
-- | Compares the precedence level of two operators. 'OpInfo' is required (and not just 'FixityInfo') because operator names are used in the FEqual case.
compareOp :: OpInfo op -> OpInfo op -> FixityOrdering
...
reassociateOpTree ::
  ...
  OpTree (Located ty) (OpSubTreeInfo (Located ty) (Located op))
reassociateOpTree getOpName opTree =
  ...
  where
    ...
    treeWithFixity = addFixityInfo defaultFixityMap (getOpName . unLoc) opTree
...
reassociateFlatNaryOpTree tree@OpBranches {noptExprs, noptOps} = case indexOfMinMaxPrecOps noptOps of
  ...
  --      forall (op1, op2) \in minOps x minOps, op1 FEqual op2
  --      forall (op1, op2) \in minOps x (opsOfCurrentLevel \ minOps), op1 FLower op2
  ...
  --      forall (op1, op2) \in maxOps x maxOps, op1 FEqual op2
  --      forall (op1, op2) \in maxOps x (opsOfCurrentLevel \ maxOps), op1 FGreater op2
      ...
      where
        ...
        go (o : os) i minOpi minRes maxOpi maxRes =
          let (minOpi', minRes') = case compareOp o minOpi of
                FEqual -> (minOpi, (:) i <$> minRes)
                FLower -> (o, Just [i])
                FGreater -> (minOpi, minRes)
                FUnknown -> (combine minOpi o, Nothing)
              (maxOpi', maxRes') = case compareOp o maxOpi of
                FEqual -> (maxOpi, (:) i <$> maxRes)
                FLower -> (maxOpi, maxRes)
                FGreater -> (o, Just [i])
                FUnknown -> (combine maxOpi o, Nothing)
              -- "Merges" two potential {min/max}Ops representatives for which the comparison gave 'FUnknown' into a representative of the {lowest/highest} precedence level encountered so far
...
setSubTreeInfo :: NaryOpTree (Located ty) (OpInfo op) -> NaryOpTree (Located ty) (OpSubTreeInfo (Located ty) op)
