...
updateModel = \case
  ...
  UpdateConfig f -> do
    #config %= f
    format
  SetShowParseResult b -> do
    #showParseResult .= b
  where
    format = do
      input <- uses #input fromMisoString
      config <- use #config
...
viewModel model@Model {..} =
            ...
            [ p_
                ...
                [ text $ "Version " <> VERSION_ormolu <> ", commit ",
                  a_
                    ...
                    [span_ [class_ "is-family-code"] [text . ms . take 7 $ commitRev]],
              ...
              div_
                ...
                [ out,
                  div_
                    ...
                    [ button_
                        [class_ "button copy-output", styleInline_ "left:90%;margin-right:20px;"]
                        [text "Copy"]
                    ]
                ]
            ]
  ...
  where
    ...
    out = case output of
      ...
      FormattingFailed e -> case fromException e of
        Just oe ->
          pre_
...
-- TODO use TH to read from file once ghc-wasm-meta contains a GHC with
-- https://gitlab.haskell.org/ghc/ghc/-/merge_requests/13466
commitRev :: String
commitRev = "deadbeef"
