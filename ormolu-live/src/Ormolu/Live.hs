...
data Output
  = FormattingFailed Text
  | FormattingSucceeded Text
  deriving stock (Show, Eq, Generic)

extractOutput :: Output -> Text
extractOutput = \case
  FormattingSucceeded t -> t
  FormattingFailed e -> e
...
updateModel = \case
  ...
  SetOutput o -> do
    ...
    zoom #outputEditor . mapAction ActionOutputEditor $
      AceEditor.updateModel outputEditorInput $
        AceEditor.SetInput (extractOutput o)
  UpdateConfig f -> do
    #config %= f
    format
  ShowParseResultChanged b -> do
    #showParseResult .= b
  CopyOutputToClipboard -> do
    output <- uses #output extractOutput
  ...
  where
    format = do
      input <- use #input
      config <- use #config
      scheduleIO . fmap SetOutput $
        tryAnyDeep (O.ormolu config "<input>" input) <&> \case
          Right t -> FormattingSucceeded t
          Left e -> FormattingFailed case fromException e of
...
viewModel model =
    ...
    [ section_
        ...
        [ div_ [class_ "container is-fluid"] . mconcat $
            [ [ h1_ [class_ "title"] [text "Ormolu Live"],
              ...
              [p_ [] [text "Loading WASM..."] | model.loading],
              [inputOutputEditors],
              [astDump | model.showParseResult],
              [ div_
  ...
  where
        ...
        [ p_
          ...
          div_ [] . intersperse (br_ []) $
            [ configCheckbox
              ...
              checkbox (^. #showParseResult) ShowParseResultChanged "Show internal parse result"
            ]
        ]
      ...
      div_
        ...
        [ div_
            ...
            [ pre_
                ...
                [ text . prettyAST model.config $ model.input
                ]
            ],
          div_
            [class_ "column"]
            [ pre_
                [class_ "is-family-code"]
                [ text . prettyAST model.config $ m
                | FormattingSucceeded m <- [model.output]
                ]
            ]
        ]
    ...
    configCheckbox (cloneLens -> l) =
      checkbox (^. #config . l) \c -> UpdateConfig $ l .~ c
...
-- Optional Wizer pre-initialization

ormoluLivePreinit :: IO ()
ormoluLivePreinit = do
  -- Evaluate fixity information
  E.evaluate $ rnf (O.hackageInfo, liveDependencies)
