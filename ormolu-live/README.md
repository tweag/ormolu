...
We use the [Miso][] library together with [jsaddle-wasm][]. Most of the code is in `src/App.hs`.

Additionally, we depend on a few packages via `npm` for JS and CSS, see `package.json`.

Finally, everything is brought together in `www/index.js`.
...
Now one can build on changes via
```console
